...
immutable OverlapIterator{Sa,Sb}
    intervals_a::Sa
    intervals_b::Sb
end

function Base.eltype{Sa,Sb}(::Type{OverlapIterator{Sa,Sb}})
    return Tuple{Interval{metadatatype(Sa)},Interval{metadatatype(Sb)}}
end

function Base.iteratorsize{Sa,Sb}(::Type{OverlapIterator{Sa,Sb}})
    return Base.SizeUnknown()
end

"""
    eachoverlap(intervals_a, intervals_b)
...
"""
function eachoverlap(intervals_a, intervals_b)
    return OverlapIterator(intervals_a, intervals_b)
end
...
function advance!(state::OverlapIteratorState, iter::OverlapIterator)
    ...
    function check_ordered(i1, i2)
        if !Bio.Intervals.isordered(i1, i2)
    ...
    while !done(state.queue, state.state_queue)
        ...
        c = compare_overlap(state.interval_a, interval_b)
        if c < 0
            if done(iter.intervals_a, state.state_a)
                break
            else
...
# Retrun -1 when `i1` precedes `i2`, 0 when `i1` overlaps with `i2`, and +1 when
# `i1` follows `i2`.
function compare_overlap(i1::Interval, i2::Interval)
