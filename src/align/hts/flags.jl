...
function decodeflag(rec::Union{Bio.Align.SAMRecord,Bio.Align.BAMRecord})
    ...
    flags_descriptions =  Dict{UInt16,String}(
        SAM_FLAG_PAIRED => "the read is paired in sequencing, no matter whether it is mapped in a pair",
        SAM_FLAG_PROPER_PAIR => "the read is mapped in a proper pair",
        SAM_FLAG_UNMAP => "the read itself is unmapped; conflictive with SAM_FLAG_PROPER_PAIR",
        SAM_FLAG_MUNMAP => "the mate is unmapped",
        SAM_FLAG_REVERSE => "the read is mapped to the reverse strand",
        SAM_FLAG_MREVERSE => "the mate is mapped to the reverse strand",
        SAM_FLAG_READ1 => "this is read1",
        SAM_FLAG_READ2 => "this is read2",
        SAM_FLAG_SECONDARY => "not primary alignment",
        SAM_FLAG_QCFAIL => "QC failure",
        SAM_FLAG_DUP => "optical or PCR duplicate",
        SAM_FLAG_SUPPLEMENTARY => "supplementary alignment"
    )

    for f in decomposeflag(rec)
        println(flags_descriptions[f])
    end
end
