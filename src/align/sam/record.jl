...
function Base.show(io::IO, record::Record)
    ...
    if isfilled(record)
        ...
        println(io, "   next reference: ", rnext(record))
        println(io, "    next position: ", pnext(record))
        println(io, "  template length: ", tlen(record))
        println(io, "         sequence: ", seq(String, record))
        println(io, "   base qualities: ", qual(String, record))
