...
@testset "Demultiplexing" begin
    ...
    barcodes = DNAKmer{8}[
        "AACACCTA", "ACTAACTG", "ATCGCCAG", "CATTCCAA", "CGCATTAA", "CTTCGCGC",
        "ACGTAGCT", "ATCCTATT", "CAGGAGGC", "CGACTGGG", "CTTAAGAT", "GCAAGTAG",
        "ATATAGGA", "CACGTGTT", "CGAACTGT", "CTCTGTCT", "GATGGAAT", "GGCAGACG",
        "CACAGTTG", "CCTTTACA", "CTAGTCAT", "GATCCAGC", "GGATATGG", "GTGACTAC",
        "CCTACAAC", "CTAGATTC", "GAGTTAAC", "GGACGAGA", "GTCTACAT", "TATACCGT",
        "CGTCGGCT", "GAGAACTC", "GCTGGCGA", "GTCCATTA", "TAGTCACA", "TGACGCAT",
        "GACGTCAA", "GCTCAGTT", "GTAGAGCT", "TAGCTAGT", "TGAATTCG", "TTCCTCAC",
        "GCGTTTCG", "GTACTTGC", "TACTGCGC", "TCGGTACC", "TTAAACAG", "AATGCTGA",
        "GGTCTGAC", "TACGAATC", "TCGCGTAC", "TGTGCTAT", "AATCACAC", "AGGTCAGT",
        "GTTTCACT", "TCCTACTA", "TGTAGGTC", "AAGATTGC", "AGGCAATG", "ATTGCATC",
        "TCCAGCCT", "TGGTCTTC", "AAGCGGTC", "AGAACACC", "ATGCATCC", "CCATACAC",
        "TGCGGTTA", "AACCGTGT", "ACTCTAAG", "ATGAGGAA", "CCAGCACG", "CGCTTCTG",
        "GCAGCCTC", "GGTCCTTG", "TGAGAGTG", "TTGAGTGT", "CAAGACCA", "CCGTCTGA",
        "GGCGAGGA", "GTTGTCCC", "TTCTGATG", "ACCGTTAT", "CCGGATAG", "CGGTTGAT",
        "GTGGGATA", "TCATTAGG", "ACAGTGCA", "AGTTGCTA", "CGGACGTG", "GACACTCT",
        "TATCTCCG", "TGATCCGA", "AGTAGTGG", "CAATCATC", "GAACGCTG", "GCGGCGAA"]

    function make_errors(seq; p=0.03, indel=true)
        seq = copy(seq)
    ...
    function test_code(code, barcodes, tolerable_failure_rate)
        @test codewords(code) == barcodes
        @test all([code[i] == barcodes[i] for i in 1:endof(barcodes)])

        # without errors
        read = DNASequence("ACGT"^15)
        n_failed = sum([demultiplex(code, DNASequence(barcodes[i]) * read) != i for i in 1:endof(barcodes)])
        @test n_failed == 0

        # with errors
        n_seqs = 10_000
        seqs = [(i, make_errors(DNASequence(barcodes[i]) * read)) for i in rand(1:endof(barcodes), n_seqs)]
        n_failed = sum([demultiplex(code, seq) != i for (i, seq) in seqs])
        @test n_failed / n_seqs < tolerable_failure_rate
    end
